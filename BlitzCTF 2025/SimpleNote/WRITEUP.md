...
However, tcache for each thread has a limit by default, 7 chunks. After the tcache is filled, new free chunks will go into the rest of the bins (the ones that \_int\_malloc looks into). These chunks can be returned by calloc. We can allocate chunks of 80 bytes at most, so we do not have to worry about any bin other than fastbins.
